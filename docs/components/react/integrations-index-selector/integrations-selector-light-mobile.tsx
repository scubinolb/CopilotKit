export function IntegrationsSelectorLightMobile({
  ...
  // Final row positions to use
  const finalRowPositions =
    rowPositions || Array.from({ length: rows }, (_, i) => getRowY(i));

  // Calculate SVG height based on row count
  const lastRowY = finalRowPositions[finalRowPositions.length - 1] || 300;
  ...
  // Generate branch paths for all but the last row (trunk handles last row)
  const branchPaths = finalRowPositions.slice(0, -1).map((y) => {
  ...
  return (
    ...
    >
      ...
      {/* Main trunk line */}
      <path d={trunkPath} stroke={`url(#${idPrefix}-trunk)`} />

      {/* Branch connectors */}
      {branchPaths.map((d, i) => (
        <path key={i} d={d} stroke={`url(#${idPrefix}-branch${i})`} />
      ))}

      <defs>
        {/* Trunk gradient - fades to right */}
        <linearGradient
          id={`${idPrefix}-trunk`}
          x1="58.6"
          y1="89.68"
        ...
        >
          ...
          <stop offset="0.9" stopColor="#BEC2FF" />
          <stop offset="1" stopColor="#BEC2FF" stopOpacity="0" />
        </linearGradient>
        {/* Branch gradients - fade to right */}
        {finalRowPositions.slice(0, -1).map((y, i) => {
          return (
            <linearGradient
              key={`branch-${i}`}
              id={`${idPrefix}-branch${i}`}
              x1="58.6"
              y1={y - 25}
              x2="81.6"
              y2={y + 12}
